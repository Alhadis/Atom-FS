...
class System{
	...
	addRequest(path, type, params = null){
		...
		if(request)
			...
			if(!this.timeoutID)
				this.timeoutID = setTimeout(() => this.sendRequests(), this.taskDelay);
			
			this.log("Job queued", opFlag, {path, params});
			return request;
		}
	}
	...
	sendRequests(){
		...
		task.on("op:done", (type, path, result) => {
			this.log("Job done", type, {path, result});
		...
		task.on("op:error", (type, path, error) => {
			this.log("Job failed", type, {path, error});
			const requestID = `${op[type].id}:${path}`;
			const request = requests.get(requestID);
			if(request){
	...
	log(status, type, params = {}){
		const timestamp = new Date();
		const title = `SYSTEM: ${status} - ${op[type].name}`;
		const entry = Object.assign({title, timestamp}, params);
